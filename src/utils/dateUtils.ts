...
const getMomentDate = (timestamp: FirebaseFirestoreTypes.Timestamp | number | Date) => {
...
export const formatMessageTime = (timestamp: FirebaseFirestoreTypes.Timestamp | number | Date) => {
...
export const formatLastSeen = (timestamp: FirebaseFirestoreTypes.Timestamp | number | Date | null) => {
  ...
  if (momentDate.isSame(now, 'year')) {
    return `Last seen on ${momentDate.format('MMM D')} at ${momentDate.format('h:mm A')}`;
  }
  
  return `Last seen on ${momentDate.format('MMM D, YYYY')} at ${momentDate.format('h:mm A')}`;
};

export const formatConversationTime = (timestamp: FirebaseFirestoreTypes.Timestamp | number | Date) => {
  const momentDate = getMomentDate(timestamp);
  const now = moment();
